...
## HTTP Transport (JWT/OAuth)

HTTP transport validates OAuth access tokens via JWKS. The user's token is forwarded directly to the DWS API — no `NUTRIENT_DWS_API_KEY` needed.
